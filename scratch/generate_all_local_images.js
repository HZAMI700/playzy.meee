...
function generateSVG(title, category, color1, color2, icon, isContent = false) {
  ...
  <defs>
    <linearGradient id="bgGrad_${category.replace(/\s+/g, '_')}" x1="0%" y1="0%" x2="100%" y2="100%">
  ...
  <rect width="1200" height="675" fill="url(#bgGrad_${category.replace(/\s+/g, '_')})" />
  ...
  <!-- Category Tag -->
  <rect x="220" y="146" width="220" height="48" rx="24" fill="#ffffff" fill-opacity="0.95" />
  <text x="330" y="176" font-family="system-ui, sans-serif" font-size="16" font-weight="900" fill="${color1}" text-anchor="middle" letter-spacing="1.5">${category} ✓</text>

  <!-- Title Text -->
  <text x="130" y="290" font-family="system-ui, sans-serif" font-size="44" font-weight="900" fill="#ffffff">${title}</text>
...
articles.forEach(art => {
  ...
  fs.writeFileSync(thumbPath, generateSVG(art.title, art.cat, art.color1, art.color2, art.icon, false));
  fs.writeFileSync(contentPath, generateSVG(`${art.title} - Benchmark`, art.cat, art.color1, art.color2, art.icon, true));
});

console.log('Successfully generated 36 local images for 18 articles!');
