  ...
  return (
    <html lang="en" className={`light ${outfit.variable} ${plusJakarta.variable}`} suppressHydrationWarning>
      ...
      <body className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 antialiased selection:bg-blue-500 selection:text-white">
        <ThemeProvider>
          <div className="flex flex-col md:flex-row min-h-screen w-full relative min-w-0">
            {/* Left Sidebar */}
            <LeftSidebar />
            ...
            <div className="flex-1 flex flex-col min-w-0 bg-white dark:bg-slate-950">
              <TopBar />
              <main className="flex-1 px-4 sm:px-8 py-6 md:py-8 bg-white dark:bg-slate-950 min-w-0">{children}</main>
              <Footer />
            </div>
          </div>
        </ThemeProvider>
