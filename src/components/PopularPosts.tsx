    ...
    <div className="p-6 rounded-3xl bg-white border border-slate-200/80 shadow-md space-y-6">
      ...
      <div className="space-y-4">
        {posts.map((post, idx) => {
          ...
          return (
            <article key={post.slug} className="flex items-start gap-3 group">
              {/* Thumbnail Image matching request */}
              {post.heroImage && (
                <div className="relative w-14 h-14 shrink-0 rounded-xl overflow-hidden bg-slate-100 border border-slate-200/60 shadow-xs">
                  <Image
                    src={post.heroImage}
                    alt={post.title}
                    fill
